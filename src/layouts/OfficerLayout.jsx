...
const NAV_ITEMS = [
  ...
  { label: "Active Election", to: "/officer/election", icon: Vote },
  { label: "Review Setup", to: "/officer/setup", icon: ClipboardList },
  { label: "Results", to: "/officer/results", icon: BarChart3 },
];

function Sidebar({ officer, onNavigate }) {
  const initials = (officer?.name || "Officer")
    .split(" ")
    .map((word) => word[0])
    .join("")
    .slice(0, 2)
    ...
    <aside className="w-[250px] bg-[#26215C] flex flex-col h-full">
      <div className="flex items-center gap-2.5 px-5 pt-5 pb-6">
        ...
        <div>
          ...
          <p className="text-[10px] text-white/35">Officer portal</p>
        </div>
      </div>
      ...
      <div className="border-t border-white/10 px-4 py-4 flex items-center gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[12px] font-medium text-[#EEEDFE] truncate">
            {officer?.name || "Election Officer"}
          </p>
          <p className="text-[10px] text-white/35 truncate">
            {officer?.email || "officer@poly.edu.ng"}
          </p>
        </div>
      </div>
    </aside>
...
function getPageMeta(pathname) {
  if (pathname === "/officer") {
    return {
      ...
      subtitle: "Overview of election oversight activity",
    };
  }

  if (pathname === "/officer/election") {
    return {
      title: "Active election",
      subtitle: "Monitor the currently open election",
    };
  }

  if (pathname === "/officer/setup") {
    return {
      title: "Review setup",
      subtitle: "Inspect positions and candidates in read-only mode",
    };
  }

  if (pathname === "/officer/results") {
    return {
      title: "Results",
      subtitle: "View published election outcomes",
    };
  }
...
export default function OfficerLayout() {
  ...
  const officer = {
    name: profile?.full_name || "Election Officer",
    email: profile?.email || "officer@poly.edu.ng",
  };
    ...
    <div className="flex h-screen overflow-hidden bg-gray-50">
      ...
      {sidebarMounted ? (
        <div className="lg:hidden fixed inset-0 z-40">
          ...
          >
            <Sidebar officer={officer} onNavigate={closeSidebar} />
          </div>
